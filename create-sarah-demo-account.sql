-- ================================================================================
-- CREATE DUPLICATE DEMO ACCOUNT FOR DR. SARAH CORTRIGHT
-- ================================================================================
-- This script copies ALL data from demo@petclaimhelper.com (Brady)
-- to drsarah@petclaimhelper.com (Dr. Sarah Cortright)
--
...
-- Replace 'SARAH_USER_ID_HERE' with the actual UUID from auth.users after creating the account
DO $$
DECLARE
  brady_user_id UUID;
  sarah_user_id UUID;
...
BEGIN
  -- Get Brady's user ID
  SELECT id INTO brady_user_id
  FROM auth.users
  WHERE email = 'demo@petclaimhelper.com';

  -- Get Sarah's user ID (you must create this auth user first!)
  SELECT id INTO sarah_user_id
  FROM auth.users
  WHERE email = 'drsarah@petclaimhelper.com';

  -- Validate both users exist
  IF brady_user_id IS NULL THEN
    RAISE EXCEPTION 'Source user demo@petclaimhelper.com not found';
  END IF;

  IF sarah_user_id IS NULL THEN
    RAISE EXCEPTION 'Target user drsarah@petclaimhelper.com not found. Create the auth user first!';
  END IF;

  RAISE NOTICE 'Brady user ID: %', brady_user_id;
  ...
  SELECT
    sarah_user_id,  -- New user ID
    'drsarah@petclaimhelper.com',  -- New email
    email_notifications,
    'Dr. Sarah Cortright',  -- New name
    phone,
    address,
    email_reminders,
    weekly_summaries,
  ...
  FOR old_pet_id IN
    SELECT id FROM public.pets WHERE user_id = brady_user_id
  LOOP
    ...
    INSERT INTO public.pets (
      ...
      filing_deadline_days, date_of_birth, breed,
      healthy_paws_pet_id, pumpkin_account_number,
  ...
  FOR old_claim_id IN
    SELECT id FROM public.claims WHERE user_id = brady_user_id
  LOOP
  ...
  FOR old_med_id IN
    SELECT id FROM public.medications WHERE user_id = brady_user_id
  LOOP
  ...
  WHERE md.user_id = brady_user_id;
  ...
  WHERE mrl.user_id = brady_user_id;
  ...
  RAISE NOTICE '================================================================================';
  RAISE NOTICE 'New demo account created for Dr. Sarah Cortright';
  RAISE NOTICE 'Email: drsarah@petclaimhelper.com';
  RAISE NOTICE 'Password: Cortright';
  RAISE NOTICE '';
  RAISE NOTICE 'Summary:';
  RAISE NOTICE '  - Profile: Copied';
  RAISE NOTICE '  - Pets: % records copied', jsonb_object_keys(pet_id_map);
  RAISE NOTICE '  - Claims: % records copied', jsonb_object_keys(claim_id_map);
  RAISE NOTICE '  - Medications: % records copied', jsonb_object_keys(medication_id_map);
