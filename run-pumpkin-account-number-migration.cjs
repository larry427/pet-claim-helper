...
async function runMigration() {
  ...
  console.log('RUNNING MIGRATION: Add pumpkin_account_number column')
  console.log('='.repeat(80))
  console.log('')

  const sql = fs.readFileSync('add-pumpkin-account-number-migration.sql', 'utf8')
  ...
  const { data: pets, error: verifyError } = await supabase
    .from('pets')
    .select('id, name, pumpkin_account_number')
    .limit(1)

  if (verifyError) {
    console.error('ERROR verifying column:', verifyError.message)
  } else {
    console.log('VERIFIED! pumpkin_account_number column exists')
