...
PRECEDENCE (READ FIRST — governs every rule below): Categorical exclusions OVERRIDE the generic coverage grants. Categorical exclusions: preventive/vaccination care; elective or cosmetic procedures (RULE 4); "not medically necessary" grooming/hygiene; preventive care WITHOUT a documented occurrence (RULE 2); waste/administrative disposal (RULE 5B); and any carrier-specific categorical exclusion stated in the policy. Generic coverage grants: RULE 4 pre/post-op inheritance for covered surgeries, and RULE 6 default-to-covered. When a line item matches BOTH a grant and a categorical exclusion, the EXCLUSION WINS — do not default it to covered — UNLESS one of these CARVE-OUT conditions is met: (1) a documented illness, injury, or occurrence drives the item; (2) a wellness endorsement applies; (3) the carrier's policy AFFIRMATIVELY covers that category of care (e.g., a base-policy preventive-care benefit such as MetGen's — see the carrier-specific rules); or (4) the visit is a SICK_INJURY_VISIT / EMERGENCY / SURGERY visit and the item is a diagnostic or prescribed medication FOR THE PRESENTING illness/injury, or a pre/post-op item of a COVERED procedure — the sick/injury visit context itself satisfies the occurrence condition, so covered-surgery inheritance (RULE 4) and sick-visit diagnostics are NOT overridden by the preventive-without-occurrence exclusion (RULE 2). Carve-out (4) does NOT reach items belonging to an unrelated ELECTIVE procedure that merely shares the visit: on a mixed bill (sick/injury treatment plus an elective procedure the same session), the elective procedure's pre-op, post-op, and support items inherit that elective procedure's EXCLUDED verdict. When no carve-out applies, an elective surgery (e.g., a routine spay/neuter with no documented medical indication) is EXCLUDED even though surgery is normally covered. INDEPENDENT EXCLUSIONS: clearing ONE categorical exclusion via a carve-out does NOT clear a different, co-applicable categorical exclusion — in particular the elective/cosmetic exclusion stands on its own: a carrier's affirmative preventive coverage (carve-out (3)) clears only the preventive-care-without-occurrence exclusion (RULE 2), NOT the elective/cosmetic one, so a routine spay/neuter with no documented medical indication remains EXCLUDED as elective on EVERY carrier, including carriers with a base-policy preventive benefit (e.g., MetGen).
...
RULE 2 — WELLNESS/PREVENTIVE (do not over-apply): "vaccination"-categorized items (vaccines and boosters) are EXCLUDED on accident/illness policies, subject to any carrier preventive-care coverage (e.g., METLIFE RULE 4). Genuinely preventive ROUTINE SCREENING performed with NO presenting symptom (e.g., a routine annual fecal or heartworm screen on a wellness visit) is EXCLUDED as preventive. IMPORTANT: the Stage 1 "preventive" category is only a HINT, not a coverage verdict. Two kinds of items are NOT preventive and must NOT be excluded under this rule:
  (i) A diagnostic test (fecal, blood panel, urinalysis, imaging, culture, etc.) used to work up the pet's PRESENTING illness or injury on a SICK_INJURY/EMERGENCY/SURGERY visit is a DIAGNOSTIC and is COVERED under the policy's Diagnostics benefit. Do NOT exclude it as "preventive" merely because its name or Stage 1 category resembles a screening test — apply RULE 2B.
  (ii) Grooming/cosmetic/hygiene items (nail trims, grooming, bathing, anal gland expression, ear cleaning that is not treating a diagnosed condition) are NOT preventive — exclude them under RULE 2C (Not Medically Necessary), not this rule.
...
- ELECTIVE or COSMETIC surgery with NO documented medical indication — a routine spay/neuter, declaw, ear crop, or tail dock on an otherwise-healthy pet — is EXCLUDED. Cite the policy's elective/cosmetic exclusion (and, where present, its preventive-care-without-occurrence exclusion — RULE 2D). Per the PRECEDENCE directive this OVERRIDES the default-covered grant. (A documented medical indication makes the surgery therapeutic — see the next bullet, which governs; a documented indication always wins.)
